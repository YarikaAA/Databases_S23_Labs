-- a) Find the maximum and minimum enrolment across all sections, considering only sections that had 
...
-- b) Find all sections that had the maximum enrolment (along with the maximum enrolment), using the
-- subquery.
...
-- c) Modify 1 to include sections with no students taking them; the enrolment for such sections should
...
-- d) Find all courses whose identifier starts with the string 'CS-1'

SELECT * 
FROM Course 
WHERE course_id LIKE 'CS-1%';

-- e) Find the names of all instructors from Biology department

SELECT name 
FROM instructor 
WHERE dept_name = 'Biology';

-- f) Find the enrolment of each section that was offered in Autumn 2022
...
-- g) Find the maximum enrolment, across all sections, in Autumn 2022
